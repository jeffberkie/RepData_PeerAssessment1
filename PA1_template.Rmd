...
The mean total number of steps per day is `r steps_mean` and the median is `r steps_median`.
...
The interval with the most steps on average each day is `r maxsteps`.
...
The total number of missing values in the dataset is `r na_sums`.
...
The mean total number of steps per day is `r new_steps_mean` and the median is `r new_steps_median`.  Because of the strategy used to impute the data for the missing 8 days, there is little change in these figures from prior to the imputation.  The mean changes by `r new_steps_mean - steps_mean` and the median changes by `r new_steps_median - steps_median`.  As is indicated by these minimal changes, the only  the impact of imputing missing data on the estimates of the total daily number of steps is to bring it  closer to the existing mean, because the only change was to add 8 days in which the total daily number of steps was equal to the mean total daily number of steps.
